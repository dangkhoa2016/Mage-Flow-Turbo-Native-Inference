...
- Two release model profiles:
  - `q8-reference` — Mage-Flow-Turbo GGUF Q8_0 diffusion, Qwen3-VL-4B GGUF Q4_K_M and required `PyTorch / vae-only`; canonical/default.
  - `bf16-safetensors` — Mage-Flow-Turbo BF16 SafeTensors `PyTorch / default` diffusion/VAE plus Qwen3-VL-4B GGUF Q4_K_M; supported alternative.
...
- Retained strict 2×2 measurements stay bound to their recorded evidence source; a checksum-protected qualification-equivalence manifest proves the final publication source preserves byte-identical qualification-critical objects without rewriting evidence provenance.
