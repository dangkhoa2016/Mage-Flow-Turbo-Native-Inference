...
- Hai release model profiles:
  - `q8-reference` — Mage-Flow-Turbo GGUF Q8_0 diffusion, Qwen3-VL-4B GGUF Q4_K_M và `PyTorch / vae-only` bắt buộc; canonical/default.
  - `bf16-safetensors` — Mage-Flow-Turbo BF16 SafeTensors `PyTorch / default` diffusion/VAE cùng Qwen3-VL-4B GGUF Q4_K_M; supported alternative.
...
- Retained strict 2×2 measurements vẫn gắn với recorded evidence source; checksum-protected qualification-equivalence manifest chứng minh final publication source giữ qualification-critical objects byte-identical mà không rewrite evidence provenance.
