...
This document defines the benchmark and interpretation contract for the first public **Mage-Flow-Turbo-Native-Inference v1.0.0** release. Fresh final measurements are produced only after the redesigned source is frozen; the measured values are published in checksum-protected GitHub Release assets/body rather than edited back into source.

## Four release cells

| Cell | Profile | Backend | Authority environment |
|---|---|---|---|
| Q8 CPU | `q8-reference` | `cpu` | fresh Kaggle `Accelerator=None` |
| BF16 CPU | `bf16-safetensors` | `cpu` | fresh Kaggle `Accelerator=None` |
| Q8 T4 | `q8-reference` | `cuda0` | fresh Kaggle T4/T4x2, physical slot 0 only |
| BF16 T4 | `bf16-safetensors` | `cuda0` | fresh Kaggle T4/T4x2, physical slot 0 only |

Every cell must use the same final source HEAD/TREE and the same canonical request. Each fresh authority session attaches only the selected Mage-Flow-Turbo diffusion family.
...
- same final source HEAD and TREE across all four cells;
...
After the four cell archives pass independent review, the frozen comparator produces:

- `mage-flow-turbo-native-inference-v1.0.0-2x2-comparison.json`
- SHA-256 sidecar for the JSON.

The frozen renderer then produces:

- `mage-flow-turbo-native-inference-v1.0.0-2x2-benchmark-summary.md`
- SHA-256 sidecar for the Markdown.

The GitHub Release body is derived from that generated summary. The README is not modified after qualification merely to paste fresh benchmark numbers.
...
Those values remain useful as pre-release research and regression context, but they are **not** final v1.0.0 exact-head authority after the 2×2 redesign. They must not be substituted for the four fresh release cells or copied into the generated final 2×2 report.

The historical T4 1024 PNG SHA-256 `2c82bdb7cd68746c113eea0f95593d4860b3a201eb3869d0c384221b39b0e49e` likewise remains historical evidence only and is not a reference hash for a fresh 512 or BF16 artifact.
